...
public class UserService {
    ...
    public UserResponse create(UserRequest req){
        ...
        if (repo.existsByCpf(req.cpf())) throw new IllegalArgumentException("CPF já cadastrado.");
    ...
    public UserResponse update(Long id, UserUpdateRequest req){
        var u = repo.findById(id).orElseThrow(() -> new NotFoundException("Usuário não encontrado."));
        if (repo.existsByEmailAndIdNot(req.email(), id)) throw new IllegalArgumentException("E-mail já cadastrado por outro usuário.");
        if (repo.existsByCpfAndIdNot(req.cpf(), id)) throw new IllegalArgumentException("CPF já cadastrado por outro usuário.");
