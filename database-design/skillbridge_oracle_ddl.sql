-- SkillBridge.AI — Oracle 19c+ DDL (para import no Oracle SQL Developer Data Modeler)
-- Observações:
--  - IDs: RAW(16) com default SYS_GUID()
--  - E-mail único case-insensitive via índice funcional em LOWER(email)
--  - JSON: CLOB com constraint IS JSON
--  - updated_at via trigger BEFORE UPDATE
--  - Sem INSERT/SEED neste arquivo (evita erro no import do Data Modeler)
...
CREATE TABLE organizations (
  ...
  created_at  TIMESTAMP(6) DEFAULT SYSTIMESTAMP NOT NULL,
  updated_at  TIMESTAMP(6) DEFAULT SYSTIMESTAMP NOT NULL,
  CONSTRAINT ck_org_type CHECK (org_type IN ('company','school','ngo','government','community'))
);
...
CREATE TABLE skills (
  ...
  CONSTRAINT fk_skills_cat FOREIGN KEY (category_id) REFERENCES skill_categories(id) ON DELETE SET NULL
);
...
CREATE OR REPLACE TRIGGER trg_usp_updated
BEFORE UPDATE ON user_skill_profiles FOR EACH ROW
BEGIN
  :NEW.updated_at := SYSTIMESTAMP;
END;
/
...
CREATE TABLE user_skills (
  ...
  CONSTRAINT ck_us_level CHECK (level BETWEEN 0 AND 5),
...
CREATE TABLE courses (
  ...
  CONSTRAINT fk_courses_org  FOREIGN KEY (org_id)    REFERENCES organizations(id) ON DELETE SET NULL,
...
CREATE TABLE enrollments (
  ...
  CONSTRAINT ck_en_status CHECK (status IN ('pending','active','completed','cancelled')),
...
CREATE TABLE mentor_availability (
  ...
  start_time     INTERVAL DAY TO SECOND, -- ou VARCHAR2(8) 'HH24:MI:SS'
  end_time       INTERVAL DAY TO SECOND,
  timezone       VARCHAR2(64) DEFAULT 'America/Sao_Paulo',
  CONSTRAINT ck_ma_wd   CHECK (weekday BETWEEN 0 AND 6),
...
CREATE TABLE mentoring_sessions (
  ...
  notes            CLOB,
  created_at       TIMESTAMP(6) DEFAULT SYSTIMESTAMP NOT NULL,
  CONSTRAINT ck_ms_status CHECK (status IN ('requested','scheduled','completed','cancelled','no_show')),
...
CREATE TABLE session_feedback (
  ...
  comment     CLOB,
  created_at  TIMESTAMP(6) DEFAULT SYSTIMESTAMP NOT NULL,
  CONSTRAINT ck_sf_rating CHECK (rating BETWEEN 1 AND 5),
...
CREATE TABLE assessment_attempts (
  ...
  CONSTRAINT fk_aa_course FOREIGN KEY (course_id) REFERENCES courses(id)        ON DELETE SET NULL,
  CONSTRAINT fk_aa_module FOREIGN KEY (module_id) REFERENCES course_modules(id) ON DELETE SET NULL
);
...
CREATE TABLE messages (
  ...
  CONSTRAINT fk_msg_session  FOREIGN KEY (session_id)       REFERENCES mentoring_sessions(id) ON DELETE SET NULL
);
...
/* Fim do DDL Oracle SkillBridge.AI */
